...
Mesh::Mesh(const aiMesh *mesh, GLint vertexLoc, GLint normalLoc, GLint texCoordLoc)
{
	...
	if( hasTexCoords )
	{
		...
		glBufferData( GL_ARRAY_BUFFER, 2 * sizeof( float ) * mesh->mNumVertices, reinterpret_cast<GLfloat *>(&texCoords[0]), GL_STATIC_DRAW );
