...
Model::Model(const std::string& filename, GLint vertexLoc, GLint normalLoc, GLint texLoc)
{
	Assimp::Importer importer;
	const aiScene* scene = nullptr;

	if( importModelFromFile( filename, importer, &scene ) )
		modelFromScene( scene, vertexLoc, normalLoc, texLoc );
}

Model::Model(const aiScene *scene, GLint vertexLoc, GLint normalLoc, GLint texLoc)
{
	modelFromScene( scene, vertexLoc, normalLoc, texLoc );
}
...
void Model::modelFromScene(const aiScene *scene, GLint vertexLoc, GLint normalLoc, GLint texLoc)
{
	hasTexCoords = true;
	for( unsigned int i = 0; i < scene->mNumMeshes; ++i )
	{
		meshes.push_back( new Mesh( scene->mMeshes[i], vertexLoc, normalLoc, texLoc ) );

		if( !meshes[i]->hasTextureCoords() )
			hasTexCoords = false;
	}
